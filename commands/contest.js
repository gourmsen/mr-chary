...
function teamContest() {
    ...
    // query attendees
    SQL = "SELECT attendeeId FROM contest_attendees WHERE contestId = ? ORDER BY attendeeId";
    DATABASE_DATA = [contestId];
    RECORDS = queryDatabase(SQL, DATABASE_DATA);

    var contestAttendees = RECORDS;

    var availableAttendees = [];
    for (var i = 0; i < contestAttendees.length; i++) {
        availableAttendees.push(contestAttendees[i].attendeeId);
    }
    ...
    // truncate round teams
    SQL = "DELETE FROM contest_attendee_teams WHERE contestId = ? AND round = ?";
    ...
    for (var i = 0; i < teamSizes.length; i++) {
        draftedTeam = getTeam(contestId, availableAttendees, teamSizes[i]);

        // write team to database
        for (var j = 0; j < draftedTeam.length; j++) {
            MODTIME = getModtime();
        ...
        // delete drafted team from available attendees
        for (var j = 0; j < draftedTeam.length; j++) {
            for (var k = 0; k < availableAttendees.length; k++) {
                if (availableAttendees[k] === draftedTeam[j]) {
                    availableAttendees.splice(k, 1);
                }
            }
        }
    }
...
function sortPairings(pairings) {
    var sortedPairings = pairings.sort((a, b) => {
        if (a.score < b.score) {
            return -1
        }
        if (a.score > b.score) {
...
function getPairings(contestId, attendees) {
    const percom = require("percom");

    var possiblePairings = percom.com(attendees, 2);

    // fill pairings
    var pairings = [];
    for (var i = 0; i < possiblePairings.length; i++) {

        // get pairing status per round (2: have paired, 1: haven't paired)
        SQL = `SELECT round, teamId, count(*) AS pairingStatus
            ...
            GROUP BY round, teamId
            ORDER BY pairingStatus DESC`;
        DATABASE_DATA = [contestId, possiblePairings[i][0], possiblePairings[i][1]];
        RECORDS = queryDatabase(SQL, DATABASE_DATA);
        ...
        // get pairing count
        var pairingCount = 0;
        for (var k = 0; k < contestAttendeeTeams.length; k++) {

            // if pairing status isn't 2, there are no more pairings
            if (contestAttendeeTeams[k].pairingStatus !== 2) {
                break;
            }
        ...
        var pairingData = {
            "first": possiblePairings[i][0],
            "second": possiblePairings[i][1],
            "pairings": pairingCount
        }

        pairings.push(pairingData);
    }

    return pairings;
}

function getTeam(contestId, attendees, teamSize) {
    const percom = require("percom");

    var possibleTeams = percom.com(attendees, teamSize);

    // calculate pairing score for each possible team
    var individualPairings = [];
    var pairingScores = [];
    var pairingScore = 0;
    for (var i = 0; i < possibleTeams.length; i++) {

        // get pairings of the current team
        individualPairings = getPairings(contestId, possibleTeams[i]);

        // calculate pairing score for the current team
        pairingScore = 0;
        for (var j = 0; j < individualPairings.length; j++) {
            pairingScore = pairingScore + individualPairings[j].pairings;
        }

        pairingScore = pairingScore / individualPairings.length;

        var pairingData = {
            "index": i,
            "score": pairingScore
        }
        pairingScores.push(pairingData)
    }

    var sortedPairings = sortPairings(pairingScores);

    // fill all pairings from lowest score pool
    var poolScore = sortedPairings[0].score;
    var pairingPool = [];
    for (var i = 0; i < sortedPairings.length; i++) {
        if (sortedPairings[i].score !== poolScore) {
            break;
        }

        pairingPool.push(sortedPairings[i]);
    }

    // pick random team from lowest score pool
